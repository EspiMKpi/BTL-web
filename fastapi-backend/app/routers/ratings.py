...
async def get_ratings(
    ...
    """Public: get ratings for a content item with aggregate stats."""
    db = get_database()
    query: dict = {"tmdb_id": tmdb_id}
    if content_type:
        query["content_type"] = content_type

    cursor = db.user_ratings.find(query).sort("created_at", -1).limit(50)
    ratings = []
    ...
    agg = await db.user_ratings.aggregate(pipeline).to_list(1)
