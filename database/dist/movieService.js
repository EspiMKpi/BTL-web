...
async function get_movie_by_id(tmdb_id) {
    try {
        ...
        const result = await Movie.findOneAndUpdate({ tmdb_id: data.id }, {
            $set: movie_document,
 ...
 */
async function get_movies_by_ids(tmdb_ids) {
 ...
 */
async function search_movies(query, limit = 20) {
    const docs = await Movie.find({ $text: { $search: query } }, { score: { $meta: "textScore" } })
