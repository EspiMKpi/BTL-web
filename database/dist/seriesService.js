 ...
 */
async function get_series_by_id(tmdb_id) {
    try {
        ...
        const response = await axios.get(`https://api.themoviedb.org/3/tv/${tmdb_id}?api_key=${process.env.TMDB_API_KEY}&append_to_response=credits`);
        const data = response.data;
        // Upsert genres
        for (const genre of data.genres || []) {
            await Genre.updateOne({ genre_id: genre.id }, { $set: { name: genre.name } }, { upsert: true });
        }
        ...
        for (const season of seasons_to_fetch) {
            try {
                const season_response = await axios.get(`https://api.themoviedb.org/3/tv/${tmdb_id}/season/${season.season_number}?api_key=${process.env.TMDB_API_KEY}`);
        ...
        const series_document = {
            ...
            networks_json: (data.networks || []),
            production_countries_json: (data.production_countries || []),
            spoken_languages_json: (data.spoken_languages || []),
            raw_data: data,
            updated_at: new Date()
        };
        const result = await Series.findOneAndUpdate({ tmdb_id: data.id }, {
            $set: series_document,
 ...
 */
async function get_series_by_ids(tmdb_ids) {
 ...
 */
async function search_series(query, limit = 20) {
    const docs = await Series.find({ $text: { $search: query } }, { score: { $meta: "textScore" } })
