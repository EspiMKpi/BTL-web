 ...
 */
async function get_home_rails(user_id) {
    const [trending_movies, trending_series, top_rated_movies, top_rated_series, new_release_movies, recent_series, genres] = await Promise.all([
    ...
    if (user_id) {
        ...
        if (continue_items.length > 0) {
            ...
            for (const m of movies)
                content_map.set(`movie_${m.tmdb_id}`, m);
            for (const s of series_list)
                content_map.set(`series_${s.tmdb_id}`, s);
 ...
 */
async function get_content_by_genre(genre_id, page = 1, limit = 20) {
...
async function get_profile_stats(user_id) {
    const [watchlist_count, completed_count, watching_count, ratings_count, history_count, avg_rating] = await Promise.all([
 ...
 */
async function get_recent_activity(user_id, limit = 20) {
