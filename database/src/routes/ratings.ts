...
router.get("/", async (req: Request, res: Response) => {
    try {
        ...
        const filter: Record<string, unknown> = { tmdb_id: Number(tmdb_id) };
...
router.get("/me", auth_middleware, async (req: Request, res: Response) => {
    try {
...
router.post("/", auth_middleware, async (req: Request, res: Response) => {
    try {
        const user_id = req.user!.user_id;
        ...
        const user_rating = await UserRating.findOneAndUpdate(
            { user_id, content_type, tmdb_id },
            {
...
router.delete("/", auth_middleware, async (req: Request, res: Response) => {
    try {
        const user_id = req.user!.user_id;
